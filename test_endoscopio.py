...
class TestEndoscopio(unittest.TestCase):
    ...
    def test_trasmision_video_con_endoscopio_apagado(self):
        with self.assertRaises(Exception):
            self.endoscopio.trasmitir_video()
    
    def test_trasmision_video(self):
        self.endoscopio.encender()
        self.assertTrue(self.endoscopio.trasmitir_video())

    def test_captura_video(self):
        self.endoscopio.encender()
        video = self.endoscopio.capturar_video(30)
        self.assertEqual(video, "video_1.mp4")

    def test_captura_video_con_duracion_exesiva(self):
        self.endoscopio.encender()
